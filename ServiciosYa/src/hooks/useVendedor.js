...
import { VendedorFindAll } from "../servicios/vendedoresServicio";
import { loadingVendedores } from "../store/slices/vendedorSlice";

export const useVendedor = () => {
    const {vendedores,vendedorSeleccionado,visibleForm,errors}= useSelector(state=>state.vendedores)
   ...
   // const { login, handlerLogout } = useAuth();
    ...
    const getVendedor = async () => {
        ...
        const result = await VendedorFindAll();
        console.log(result);
        dispatch(loadingVendedores(result.data));
          
     } catch (error) {
            
    }
}
const handlerRemoveVendedor = (id) => {
